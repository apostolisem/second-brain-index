...
class OneNoteTopicsError(Exception):
    pass


def load_onenote_topics(path: Path) -> dict[str, list[str]]:
    if not path.exists():
        return {}
    try:
        content = path.read_text(encoding="utf-8")
    except OSError as exc:
        raise OneNoteTopicsError(f"Unable to read {path.name}: {exc}") from exc
    try:
        data = json.loads(content)
    except json.JSONDecodeError as exc:
        raise OneNoteTopicsError(f"Invalid JSON in {path.name}: {exc}") from exc
...
def infer_states_from_paths(paths: list[str], topic_name: str) -> set[str]:
    states: set[str] = set()
    for path in paths:
        state = _state_from_onenote_path(path)
        if state:
            states.add(state)
    if not states:
...
def _state_from_onenote_path(path: str) -> str | None:
